...
use tokio::{
    net::{UdpSocket, ToSocketAddrs, lookup_host},
    time::{timeout, Duration},
};

use super::{
    time::{Delay, Timestamp, now_microseconds},
    packet::{Packet, PacketType, ExtensionType, HEADER_SIZE},
    util::{generate_sequential_identifiers, abs_diff, ewma},
    error::SocketError
};

// For simplicity's sake, let us assume no packet will ever exceed the
// Ethernet maximum transfer unit of 1500 bytes.
pub(in crate) const BUF_SIZE: usize = 1500;
...
impl UtpSocket {
    ...
    pub(in crate) fn from_raw_parts(s: UdpSocket, src: SocketAddr) -> UtpSocket {
    ...
    ///
    /// If more than one valid address is specified, only the first will be used.
    pub async fn connect<A: ToSocketAddrs>(to: A) -> Result<Self> {
        let addr = take_address(to).await?;
        ...
        for _ in 0..MAX_SYN_RETRIES {
            ...
            // Validate response
            match timeout(syn_timeout, socket.socket.recv_from(&mut buf)).await
    ...
    pub async fn recv_from(&mut self, buf: &mut [u8]) -> Result<(usize, SocketAddr)> {
        ...
        loop {
            ...
            if self.state == SocketState::Closed {
                println!("test {:?}", self.connected_to);
    ...
    async fn recv(&mut self, buf: &mut [u8]) -> Result<(usize, SocketAddr)> {
        ...
        loop {
            ...
            let ms = if self.state != SocketState::New {
            ...
            let response = match ms {
                Some(ms) => timeout(ms, self.socket.recv_from(&mut b)).await
                    .map_err(|e| io::Error::new(ErrorKind::TimedOut, e))
                    .flatten(),
        ...
        let packet = match Packet::try_from(&b[..read]) {
            Ok(packet) => packet,
            Err(_) => return Ok((0, self.connected_to)),
        };
    ...
    pub(in crate) async fn handle_packet(&mut self, packet: &Packet, src: SocketAddr) -> Result<Option<Packet>> {
...
/*impl Drop for UtpSocket {
    fn drop(&mut self) {
        ...
        let _ = handle.block_on(self.close());
    }
}*/
...
mod tests {
    ...
    macro_rules! iotry {
        ($e:expr) => (match $e.await { Ok(e) => e, Err(e) => panic!("{:?}", e) })
    }
    ...
    async fn test_socket_ipv4() {
        ...
        let child = tokio::spawn(async move {
            ...
            assert_eq!(
                client.connected_to,
                take_address(server_addr).await.unwrap()
            );
    ...
    async fn test_handle_packet() {
        ...
        let (server_addr, client_addr) = (
            take_address(next_test_ip4()).await.unwrap(),
            take_address(next_test_ip4()).await.unwrap(),
        );
    ...
    async fn test_tolerance_to_small_buffers() {
        ...
        while server.state != SocketState::Closed {
            let mut small_buffer = vec![0; 512];
    ...
    async fn test_local_addr() {
        let addr = next_test_ip4();
        let addr = take_address(addr).await.unwrap();
    ...
    async fn test_peer_addr() {
        ...
        let addr = next_test_ip4();
        let server_addr = take_address(addr).await.unwrap();
        ...
        let child = tokio::spawn(async move {
            ...
            tx.send(client.local_addr()).await.unwrap();
        ...
        // correct
        let client_addr = rx.recv().await.unwrap().unwrap();
